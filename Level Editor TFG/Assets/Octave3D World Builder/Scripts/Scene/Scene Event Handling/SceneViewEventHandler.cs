    ...
    public abstract class SceneViewEventHandler
    {
        ...
        public void HandleSceneViewEvent(Event e)
        {
            if(e.IsUndoRedo())
            {
                ...
                if (ObjectPlacementSettings.Get().HideWireframeWhenPlacingObjects) Octave3DWorldBuilder.ActiveInstance.HideWireframeForSceneObjectsAndPlacementGuide();
            ...
            switch(e.type)
            {
                ...
                case EventType.KeyDown:
                    ...
                    if (e.type == EventType.KeyDown && e.keyCode == KeyCode.Delete) e.DisableInSceneView();
        ...
        protected virtual bool CanEventBeHandled(Event e)
        {
            ...
            // When the ALT key is pressed, we want to allow the user to orbit the camera
            if (e.alt && numberOfPressedMouseButtons != 0) return false;
            ...
            if (e.type == EventType.KeyDown && numberOfPressedMouseButtons != 0)
            {
                if (e.keyCode == KeyCode.W || e.keyCode == KeyCode.Q ||
                    ...
                    e.keyCode == KeyCode.S || e.keyCode == KeyCode.D) return false;
            }

            // Note: Input gets blocked if we press a key while another mouse button is alos pressed. For this
            //       reson, we will not allow 'KeyDown' events to be processed if at least one mouse button is
            //       pressed.
