    ...
    public class ObjectPlacement : ScriptableObject, IMessageListener
    {
        ...
        public void RenderHandles()
        {
            if (ObjectPlacementGuide.ExistsInScene)
            {
                ...
                GUILayout.Label(" Active prefab: " + ObjectPlacementGuide.Instance.SourcePrefab.Name, labelStyle);
            }
        ...
        public void HandleKeyboardButtonDownEvent(Event e)
        {
            ...
            if (ObjectPlacementGuide.ExistsInSceneAndIsActive)
            {
                ...
                if(AllShortcutCombos.Instance.ToggleIgnoreObjectsWhenSnapping.IsActive())
                {
                    e.DisableInSceneView();
                    UndoEx.RecordForToolAction(ObjectSnapSettings.Get());
                    ObjectSnapSettings.Get().IgnoreObjectsWhenSnapping = !ObjectSnapSettings.Get().IgnoreObjectsWhenSnapping;
                ...
                /*else
                if(e.keyCode == KeyCode.U && ObjectPlacementGuide.ExistsInSceneAndIsActive)
                {
                    e.DisableInSceneView();

                    float xCellSize = ObjectSnapping.Get().XZSnapGrid.CellSizeSettings.CellSizeX;
                    float zCellSize = ObjectSnapping.Get().XZSnapGrid.CellSizeSettings.CellSizeZ;
                    var localAxes = ObjectPlacementGuide.SceneObject.GetLocalAxes();

                    Vector3 gridRight = ObjectSnapping.Get().XZSnapGrid.TransformMatrix.GetNormalizedAxis(0);
                    Vector3 gridLook = ObjectSnapping.Get().XZSnapGrid.TransformMatrix.GetNormalizedAxis(2);

                    int rightAlignedAxis = Vector3Extensions.GetIndexOfMostAlignedVectorIgnoreSign(localAxes, gridRight);
                    int lookAlignedAxis = Vector3Extensions.GetIndexOfMostAlignedVectorIgnoreSign(localAxes, gridLook);

                    if(rightAlignedAxis >= 0 && lookAlignedAxis >= 0)
                    {
                        OrientedBox guideOOBB = ObjectPlacementGuide.Instance.SourcePrefab.UnityPrefab.GetHierarchyWorldOrientedBox();
                        Vector3 scaleFactor = Vector3.one;

                        Transform guideTransform = ObjectPlacementGuide.SceneObject.transform;
                        guideOOBB.Scale = guideTransform.lossyScale;

                        scaleFactor[rightAlignedAxis] = xCellSize / guideOOBB.ScaledSize[rightAlignedAxis];
                        scaleFactor[lookAlignedAxis] = zCellSize / guideOOBB.ScaledSize[lookAlignedAxis];

                        ObjectPlacementGuide.Instance.SetWorldScale(Vector3.Scale(scaleFactor, guideTransform.lossyScale));
                    }
                }*/
            }
        ...
        {
            if (objectPlacementType == ObjectHierarchyRootsWerePlacedInSceneMessage.PlacementType.MirroredSelection) return false;
        ...
        private bool CanAttachPlacedRootsToObjectGroup(ObjectHierarchyRootsWerePlacedInSceneMessage.PlacementType objectPlacementType)
        {
            ...
            if (objectPlacementType == ObjectHierarchyRootsWerePlacedInSceneMessage.PlacementType.MirroredSelection) return ObjectSelection.Get().Settings.AttachMirroredObjectsToActiveObjectGroup;
