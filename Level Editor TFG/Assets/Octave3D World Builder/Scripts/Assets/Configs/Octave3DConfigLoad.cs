        ...
        private static void LoadSnapSettings(XmlDocument xmlDoc, Octave3DConfigSaveLoadSettings loadSettings)
        {
            if(loadSettings.SnapSettings)
            {
                ...
                if(snapSettingsNode != null)
                {
                    ...
                    node = snapSettingsNode.SelectSingleNode(".//" + Octave3DConfigXMLInfo.IgnoreObjectsWhenSnappingNode);
                    if (node != null)
                    {
                        bool boolRes;
                        if (bool.TryParse(node.InnerText, out boolRes)) snapSettings.IgnoreObjectsWhenSnapping = boolRes;
                    }
                    ...
                    node = snapSettingsNode.SelectSingleNode(".//" + Octave3DConfigXMLInfo.ObjectToObjectSnapModeNode);
                    if (node != null)
                    {
                        try
                        {
                            snapSettings.ObjectToObjectSnapMode = (ObjectToObjectSnapMode)Enum.Parse(typeof(ObjectToObjectSnapMode), node.InnerText);
                        }
                        catch (Exception) { }
                    }
        ...
        private static void LoadObjectSelectionSettings(XmlDocument xmlDoc, Octave3DConfigSaveLoadSettings loadSettings)
        {
            if(loadSettings.ObjectSelectionSettings)
            {
                ...
                if(selectionSettingsNode != null)
                {
                    ...
                    node = selectionSettingsNode.SelectSingleNode(".//" + Octave3DConfigXMLInfo.AttachMirroredSelectedObjectsToActiveGroupNodeNode);
                    if (node != null)
                    {
                        bool boolRes;
                        if (bool.TryParse(node.InnerText, out boolRes)) objectSelectionSettings.AttachMirroredObjectsToActiveObjectGroup = boolRes;
                    }
